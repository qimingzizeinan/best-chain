...
import { importAsync, errorStep as errorStep$2, info as info$1, npm } from '@best-chain/utils';
...
var license = "MIT";
var dependencies = {
	"@best-chain/utils": "workspace:^1.0.0",
	cac: "^6.7.14",
	compressing: "^1.7.0",
	dotenv: "^16.0.3",
	execa: "^7.0.0",
...
const DEFAULT_CLI_HOME = '.best-chain';
 ...
 * @param {*} _file
 */
const resolvePath = (_path, _file) => path$c.resolve(_path, _file);
/**
 * Select environment to deploy.
 * @param {*} CONFIG configuration
 */
function selectDeployEnv(servers) {
...
async function getConfig() {
    ...
    if (!fs$i.existsSync(configFile)) {
        errorStep$1(`${configFile} does not exist.`);
    ...
    try {
        ...
        if (configError) {
            errorStep$1('Incorrect configuration！\n');
 ...
 * @param {*} next
 */
async function compressFiles(localConfig) {
    try {
        const { distDir, distZip } = localConfig;
        const dist = resolvePath(process.cwd(), distDir);
        if (!fs$i.existsSync(dist)) {
            errorStep$1('× compress fail！');
            errorStep$1(`× [local.distDir] path is incorrect configuration！ ${dist} is not exist！\n`);
            return;
        }
        ...
        errorStep$1(`Compression failure！ \n error:${JSON.stringify(err)}`);
    }
}
/**
 * Connection server
 ...
 */
async function connectServer(connectConfig) {
 ...
 * @param next
 */
async function deploy(localConfig, serverConfig) {
    const { host, port, username, password, distDir, distZipName, backup } = serverConfig;
    if (!host || !username || !password || !distDir || !distZipName) {
        errorStep$1(`${configFileName}.json incorrect configuration！`);
        process.exit(1);
    }
    if (!distDir.startsWith('/') || distDir === '/') {
        errorStep$1(`[server.distDir: ${distDir}] Must be an absolute path and cannot be the root directory "/"`);
        process.exit(1);
    }
    // Connection server
    ...
    try {
        ...
        await SSH?.putFile(resolvePath(process.cwd(), localConfig.distZip), `${distDir}/${distZipName}.zip`).catch((err) => {
...
async function runCommandAtServer(cmd, cwd) {
    await SSH?.execCommand(cmd, {
        cwd,
        onStderr(chunk) {
            errorStep$1(`${cmd}, stderrChunk, ${chunk.toString('utf8')}`);
        }
    });
}
/**
...
async function start() {
    await createSSH();
    step$1('getting configuration...');
    const c = await getConfig();
    if (!c)
        return;
    const config = await selectDeployEnv(c.servers);
    if (!config) {
        errorStep$1('not found configuration');
        return;
    }
...
// 压缩assets并且部署到服务器
async function assetsCompressAndDeploy() {
    await createSSH();
...
// 强制更新证书
async function fastlaneMatchForce_match() {
    try {
...
// 热更包流程
async function hotUpdateBag() {
    await modifyVersion();
    await cocosBuildIOS();
}
async function cocosDeploy(type) {
    const c = await getConfig();
    if (!c)
        return;
    config = c;
...
function startCommand() {
    ...
    cli
        .command('cocos [type]', 'Deployment cocos project. type = [base | b | hot | h | push | p | match | m | platform]')
        .action(async (type) => {
        if (!type) {
            errorStep$2('Command error! example: bci cocos base');
            return;
        }
        cocosDeploy(type);
    });
...
function showCliVersion() {
    info$1(`best-chain cli version: ${packageConfig.version}. Welcome to use`);
}
function checkUserHome() {
    if (!userHome || !require$$0$3.existsSync(userHome)) {
...
function checkEnv() {
    dotenv.config({
        path: require$$1.resolve(userHome, '.env')
    });
...
function createCliConfig() {
    ...
    if (process.env.CLI_HOME) {
        cliConfig['cliHome'] = require$$1.join(userHome, process.env.CLI_HOME);
    }
    else {
        cliConfig['cliHome'] = require$$1.join(userHome, DEFAULT_CLI_HOME);
    }
    return cliConfig;
}
async function checkLocalCliUpdate() {
    ...
    try {
        ...
        if (lastVersion && semver.gt(lastVersion, currentVersion)) {
            errorStep$2(yellow(`Please update manually. package name: ${packageConfig.name}，Current version：${packageConfig.version}，Latest version：${lastVersion}
